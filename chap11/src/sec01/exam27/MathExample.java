...
public class MathExample {
	...
	public static void main(String[] args) {
		...
		// 소수점 이하 특정 자리에서 반올림 하기
		
		double value = 12.3456;
		try (Scanner sc = new Scanner(System.in)) {
			System.out.println("몇번째 자리에서 반올림 할까요?");
			int num = sc.nextInt();
			double scale = Math.pow(10, num-1);
			double multipleValue = value * scale;
			double roundValue = Math.round(multipleValue) / scale;
			System.out.println(num + "자리에서 반올림 결과: " + roundValue);
