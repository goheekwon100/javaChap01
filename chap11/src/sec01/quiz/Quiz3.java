...
//	fromIndex: 검색 시작 위치 지정
	...
	public static void main(String[] args) {
		...
		String[] targetStrArr = new String[3];
		
		try (Scanner sc = new Scanner(System.in)) {
			for (int i = 0; i < 3; i++) {
				System.out.print((i+1) + ") ");
				strArr[i] = sc.next();
				targetStrArr[i] = sc.next();
			}
			for (int i = 0; i < strArr.length; i++) {
				int num = count(strArr[i], targetStrArr[i]);
				System.out.println((i+1) + ") " + num);
			}
		}
	}
	
	public static int count(String currentStr, String targetStr) {
		int result=0;
		int index = 0;
		while(true) {
			int searchStringIndex = currentStr.indexOf(targetStr, index);
			if (searchStringIndex == -1) {
				break;
			}else {
				index = searchStringIndex+1;
				result++;
			}
